...
<ul class="sidebar-menu" id="root_menu">
    ...
    <li class=""><a href="http://store.whatsns.com/#/" target="_blank"><i class="fa fa-shopping-cart"></i> <span class="red">应用商店</span> </a></li>
    {if $user['groupid']==1}
    ...
    {eval  $adminnavlist=$this->getlistbysql("select * from ".$this->db->dbprefix."admin_nav where status=1 and pid=0  order by ordernum asc  limit 0,100");}
      ...
      {/if}
        ...
        <li {if $regular==$chlidnav['url']}class="current"{/if}><a href="{$chlidnav['suburl']}" target="main"><i class="fa fa-genderless text-success"></i>{$chlidnav['name']}</a></li>
      {/loop}
        </ul>
    </li>
    {/loop}
